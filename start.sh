#!/bin/bash
#
# Start script for item-group-status-updater
...
if [[ -z "${MESOS_SLAVE_PID}" ]]; then
    ...
    source ~/.chs_env/item-group-status-updater/env

    PORT="${ITEM_GROUP_STATUS_UPDATER_PORT}"
else
    PORT="$1"
    CONFIG_URL="$2"
    ENVIRONMENT="$3"
...
exec java ${JAVA_MEM_ARGS} -jar -Dserver.port="${PORT}" "${APP_DIR}/item-group-status-updater.jar"